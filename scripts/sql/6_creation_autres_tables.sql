...
-- Table quantité : stocke la quantité totale d'un département et permet le lien avec les quantités détaillées par categorie/cepage de vin
...
-- Table quantité détaillée : stocke les quantités détaillés directement en lien avec le categorie et le cepage de vin
CREATE TABLE t_quantite_detail (
  ...
  CONSTRAINT pk_id_quantite_detail PRIMARY KEY(id_quantite_detail),
    ...
    CONSTRAINT fk_quantite_detail_cepage FOREIGN KEY(id_cepage) REFERENCES t_cepage(id_cepage)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;